...
resource "null_resource" "node_command_node1" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "${rancher2_cluster_v2.downstream.cluster_registration_token[0].insecure_node_command} --etcd --controlplane --worker"
    ]
  }
}
...
resource "null_resource" "node_command_node2" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "${rancher2_cluster_v2.downstream.cluster_registration_token[0].insecure_node_command} --worker"
    ]
  }
}
